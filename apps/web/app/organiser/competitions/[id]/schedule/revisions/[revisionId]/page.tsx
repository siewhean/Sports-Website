...
export default async function ScheduleRevisionPage({
  params,
}: {
  ...
  return (
    <OrganiserWorkspace
      ...
      sectionAction={null}
      pageTitle={`${phase4ScheduleCopy.draft} ${revision.revision}`}
      pageIntro={phase4ScheduleCopy.immutablePrivateHistory}
      pageEyebrow={phase4ScheduleCopy.scheduleRevision}
      syncLabel={revision.status.replaceAll("_", " ")}
      syncState={
        revision.status === phase4ScheduleMachine.expired ? phase4ScheduleMachine.readOnly : phase4ScheduleMachine.saved
      }
      sectionContent={<ScheduleRevisionDetail document={document} revision={revision} />}
    />
  );
}
