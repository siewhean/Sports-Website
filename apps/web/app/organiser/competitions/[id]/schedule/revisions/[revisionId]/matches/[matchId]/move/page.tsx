...
export default async function ScheduleMovePage({ params }: { params: Promise<{ id: string; revisionId: string; matchId: string }> }) {
  ...
  if (!requested || (requested.status !== phase4ScheduleMachine.draft && requested.status !== phase4ScheduleMachine.readyForReview)) notFound();
  const selectedDocument = requested.id === document.currentRevision?.id ? document : { ...document, currentRevision: requested, canPublish: false };
