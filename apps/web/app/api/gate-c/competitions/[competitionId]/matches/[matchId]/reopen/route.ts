...
export async function POST(
  ...
  if (
    !body ||
    !hasExactKeys(body, machine.reopenBodyKeys) ||
    ...
    return NextResponse.json(
      { error: { code: machine.reopenCommandInvalid, message: copy.reopenInvalid } },
      { status: 400 },
    );
  }
  return forwardPhase3Mutation(request, {
    method: machine.post,
    path: `/api/v1/competitions/${encodeURIComponent(competitionId)}/matches/${encodeURIComponent(matchId)}/reopen`,
    body: {
