...
export async function GET(request: NextRequest, { params }: { params: Promise<{ competitionId: string }> }) {
  ...
  return forwardScheduleRead(request, `/api/v1/competitions/${encodeURIComponent(competitionId)}/schedule-jobs`);
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ competitionId: string }> }) {
  ...
  if (
    ...
    !hasExactKeys(body, [
      ...
      phase4ScheduleMachine.expectedSourceRevisionField,
      phase4ScheduleMachine.expectedCapacityRevisionField,
      phase4ScheduleMachine.objectiveField,
      phase4ScheduleMachine.constraintsField,
    ]) ||
    typeof body.idempotency_key !== "string" ||
    !Number.isSafeInteger(body.expected_source_revision) ||
    !Number.isSafeInteger(body.expected_capacity_revision) ||
    ![phase4ScheduleMachine.fastest, phase4ScheduleMachine.balanced, phase4ScheduleMachine.restFocused].includes(
      String(body.objective) as never,
    ) ||
    !body.constraints ||
  ...
  return forwardPhase3Mutation(request, {
    ...
    path: `/api/v1/competitions/${encodeURIComponent(competitionId)}/schedule-jobs`,
    body,
    validate: (value) => parseScheduleJobEnvelope(value) !== null,
  });
}
