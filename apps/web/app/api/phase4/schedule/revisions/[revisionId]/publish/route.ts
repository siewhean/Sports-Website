...
export async function POST(request: NextRequest, { params }: { params: Promise<{ revisionId: string }> }) {
  ...
  return forwardPhase3Mutation(request, { method: phase4ScheduleMachine.post, path: `/api/v1/schedule-revisions/${encodeURIComponent(revisionId)}/publish`, body, validate: (value) => parseScheduleRevisionView(value, true, true) !== null });
}
