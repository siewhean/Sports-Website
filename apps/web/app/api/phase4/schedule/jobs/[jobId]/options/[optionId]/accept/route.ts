...
export async function POST(request: NextRequest, { params }: { params: Promise<{ jobId: string; optionId: string }> }) {
  ...
  if (
    !body ||
    !hasExactKeys(body, [phase4ScheduleMachine.idempotencyKeyField, phase4ScheduleMachine.expectedJobRevisionField]) ||
    typeof body.idempotency_key !== "string" ||
    !Number.isSafeInteger(body.expected_job_revision)
  )
  ...
  return forwardPhase3Mutation(request, {
    ...
    path: `/api/v1/schedule-jobs/${encodeURIComponent(jobId)}/options/${encodeURIComponent(optionId)}/accept`,
    body,
