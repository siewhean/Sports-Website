...
import { setupSportLabel } from "@/lib/phase4-assisted-setup-flow";
...
const copy = {
  ...
  active: opaqueId("active"),
  minuteSlotsIn: opaqueId("minute slots in"),
  selectedSport: opaqueId("Selected sport"),
  customised: opaqueId("Customised"),
  recommended: opaqueId("Recommended"),
  immutableSettingsReferences: opaqueId("immutable settings references"),
  competitionSettings: opaqueId("Competition settings"),
  divisionSettings: opaqueId("Division settings"),
  total: opaqueId("total"),
  divisions: opaqueId("divisions"),
  imports: opaqueId("imports"),
  confirmedLower: opaqueId("confirmed"),
  placeholdersRevision: opaqueId("placeholders · revision"),
  capacityRevision: opaqueId("Capacity r"),
...
export function AssistedSetupJourneyView({
  ...
  viewState: AssistedSetupSurfaceState;
  commandBusy: boolean;
  autosaving: boolean;
  announcement: string;
  basics: BasicsDraft | null;
  preferences: PreferencesDraft | null;
  brief: string;
  briefMessage: string;
  missingFields: readonly string[];
  headingRef: RefObject<HTMLHeadingElement | null>;
  onBasics(value: BasicsDraft): void;
  ...
  if (viewState !== "ready" && viewState !== "read-only") {
    const surface = stateCopy[viewState];
    return (
      <SetupState
    ...
    <div className={styles.workspace} data-testid="phase4-assisted-setup">
      ...
      <section className={styles.main}>
        ...
        <div className={styles.stepBody}>
          {setup.current_step === "basics" ? (
            basics ? (
              <BasicsStep
                value={basics}
                onChange={onBasics}
                brief={brief}
                onBriefChange={onBrief}
                onConvert={onConvertBrief}
          ...
          {setup.current_step === "capacity" ? (
            <CapacityStep setup={setup} competitionId={document.competitionId} />
          ) : null}
          {setup.current_step === "settings" ? (
            <SettingsStep setup={setup} competitionId={document.competitionId} />
          ) : null}
          {setup.current_step === "entries" ? (
            <EntriesStep setup={setup} competitionId={document.competitionId} />
          ) : null}
          {setup.current_step === "format_preferences" ? (
            preferences ? (
          ...
          {setup.current_step === "format_recommendations" ? (
            <RecommendationStep setup={setup} disabled={disabled} onSelect={onSelectRecommendation} />
          ) : null}
        ...
        <footer className={styles.actions}>
          ...
          {currentIndex < assistedSetupSteps.length - 1 ? (
            <button className={styles.continue} type="button" disabled={disabled} onClick={onContinue}>
              {commandBusy
                ? t("prototype.23e39291d613")
                : `${copy.continueTo} ${assistedSetupSteps[currentIndex + 1]!.label.toLowerCase()}`}
  ...
  return (
    <div className={styles.basicsLayout}>
      ...
      <div className={styles.fieldGrid}>
        ...
        <Field id="setup-sport" label={t("prototype.9a085dd854f0")} helper={copy.sportReset}>
          <select
        ...
        <Field id="setup-locality" label={copy.locality}>
          <input
        ...
        <Field id="setup-country-code" label={copy.countryCode}>
          <input
        ...
        <Field id="setup-time-zone" label={copy.timezone}>
          <input
        ...
        <Field id="setup-locale" label={copy.locale}>
          <input
            value={value.locale}
        ...
        <Field id="setup-entry-status" label={copy.entryStatus}>
          ...
          >
            ...
            <option value="estimated">{copy.estimated}</option>
          </select>
        </Field>
      </div>
    </div>
    ...
    <div className={styles.capacityLayout}>
      <div className={styles.referenceRows}>
        <ReferenceRow
          ...
          value={`${capacity.area_ids.length} ${copy.active}`}
        ...
        <ReferenceRow
          ...
          detail={`${capacity.effective.slotMinutes} ${copy.minuteSlotsIn} ${capacity.time_zone}`}
        />
        <ReferenceRow
          icon={<Warning />}
...
function SettingsStep({ setup, competitionId }: { setup: Phase4SetupDocument; competitionId: string }) {
  ...
  const sport = setup.values.basics ? setupSportLabel(setup.values.basics.sport_code) : copy.selectedSport;
  if (!values.length)
    return (
      <InlineEmpty
  ...
  return (
    <div className={styles.referenceRows}>
      <ReferenceRow
        icon={<ShieldCheck />}
        title={`${sport} · ${copy.canonicalRules}`}
        value={values.some((item) => item.mode === "customised") ? copy.customised : copy.recommended}
        detail={`${values.length} ${copy.immutableSettingsReferences}`}
      />
      {values.map((item) => (
        <ReferenceRow
          key={`${item.scope}:${item.division_id ?? "all"}`}
          icon={<ShieldCheck />}
          title={item.scope === "competition" ? copy.competitionSettings : copy.divisionSettings}
          value={item.mode === "recommended" ? copy.recommended : copy.customised}
  ...
  return (
    <div className={styles.referenceRows}>
      <ReferenceRow
        ...
        value={`${entries.total_entry_count} ${copy.total}`}
        detail={`${entries.divisions.length} ${copy.divisions} · ${entries.imports.length} ${copy.imports}`}
      />
      {entries.divisions.map((division, index) => (
        <ReferenceRow
          ...
          value={`${division.confirmed_count} ${copy.confirmedLower}`}
          detail={`${division.placeholder_count} ${copy.placeholdersRevision} ${division.division_revision}`}
        />
      ))}
  ...
  return (
    <div className={styles.referenceRows}>
      ...
      <ReferenceRow
        ...
        value={`${copy.capacityRevision}${schedule.capacity_revision}`}
        detail={`${schedule.settings_references.length} ${copy.settingsReferences}`}
      />
