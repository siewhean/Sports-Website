...
test("phase 4 move flow visual baselines", async ({ page }, testInfo) => {
  ...
  await page.route("**/moves/validate", async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ validation: { valid: true, violations: [] }, assignments: [], consequences: { moved_match_id: "match", from: null, to: null, affected_match_ids: ["match"], dependency_match_ids: [], locked_match_ids: [], messages: ["Only the selected slot changes."], quality: null } }) }));
  await page.goto("/organiser/competitions/singapore-open/schedule/revisions/70000000-0000-4000-8000-000000000004/matches/30000000-0000-4000-8000-000000000001/move");
  await dismissConsent(page);
  await hidePrivacyControl(page);
...
async function hidePrivacyControl(page: import("@playwright/test").Page) {
  ...
  if (await control.isVisible()) await control.evaluate((element) => { element.setAttribute("hidden", ""); });
}
