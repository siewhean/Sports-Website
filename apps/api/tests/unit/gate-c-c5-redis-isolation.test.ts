...
describe("C5 Redis ownership contract", () => {
  it("owns only the exact Bull, dead-letter, cancellation, and rate-limit namespaces", () => {
    ...
    expect(ownership.ownedPatterns).toEqual([
      "matchday-c5-queue-a:matchday-foundation:*",
      "matchday-c5-queue-a:matchday-foundation.dead-letter:*",
      "matchday:job-cancellation:matchday-c5-queue-a:matchday-foundation:*",
      "matchday:test:gate-c-c5:run-a:*",
    ]);
    expect(ownership.unrelatedGuardKey).toBe("matchday-c5-queue-a:matchday-foundation-foreign:ttl-sentinel");
  });

  it.each([
    ["bull", "matchday-foundation", "matchday:test:gate-c-c5:run-a:"],
    ["matchday-c5-queue-a", "foundation", "matchday:test:gate-c-c5:run-a:"],
    ["matchday-c5-queue-a", "matchday-foundation", "matchday:test:scoring-access:run-a:"],
