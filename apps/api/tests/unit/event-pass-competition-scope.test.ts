...
import type { StripeCheckoutSessionParams } from "../../src/stripe-checkout-client.js";

describe("Event Pass competition scope", () => {
  it("binds Event Pass checkout to an owned competition", async () => {
    const captured: StripeCheckoutSessionParams[] = [];
    const sql = {
    ...
    const stripe = {
      createSession: async (params: StripeCheckoutSessionParams) => {
  ...
  it("rejects an Event Pass webhook without a competition target", async () => {
    const secret = "whsec_event_pass_scope";
    const now = Math.floor(Date.now() / 1000);
    const calls: string[] = [];
    const sql = {
    ...
    const runtime = new EntitlementRuntime(sql);
    const payload = {
      id: "evt_event_pass_without_competition",
      type: "checkout.session.completed",
      data: {
        object: {
    ...
    await expect(runtime.processBillingWebhook(`t=${now},v1=${signature}`, raw, payload, secret)).rejects.toMatchObject(
      {
        statusCode: 422,
