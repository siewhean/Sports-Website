...
const ScheduleAdjustmentBody = strict({
  match_id: Id,
  division_id: Id,
...
const CreateRevisionBody = strict({
  ...
  decisions: Type.Array(DecisionBody, { maxItems: 100 }),
  schedule_adjustments: Type.Array(ScheduleAdjustmentBody, { maxItems: 100 }),
});
...
export async function registerGateCC4Routes(
  ...
  app.get<{ Params: { competitionId: string } }>(
    ...
    async (request) => options.lifecycle.listPendingCases(await readActor(request), request.params.competitionId),
  );
  ...
  app.post<{
    ...
    Body: Static<typeof CreateRevisionBody>;
  }>(
    ...
    async (request, reply) =>
      reply.code(201).send(
        await options.runtime.createRevision(
          ...
          {
            ...request.body,
            parent_revision_id: request.body.parent_revision_id ?? null,
          },
          request.id,
        ),
      ),
  ...
  app.post<{
    ...
    async (request) => {
      if (
        ...
        throw new ApiError(400, ErrorCode.REQUEST_INVALID, "Publication path and body identifiers must match");
      }
