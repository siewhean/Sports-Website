...
| `GATE_D_STAGING_PUBLIC_RATE_LIMIT_MAX`                    | Optional Gate D benchmark-only anonymous API limit. Accepted **only** when `APP_ENV=staging`, must be 600–10,000 requests/minute, and must never be set in local, test, or production. |
