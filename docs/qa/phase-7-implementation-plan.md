...
| QA-010 | Load-test public pages                                           | `apps/api/scripts/run-load-public.ts`                                                                                                                         | Component/socket-component runs are developer diagnostics only; exact deployed API/PostgreSQL/Redis receipt remains required | Staging p95 `<2.5s`, error rate `<=0.1%`, current SLO version, time window, sample/session thresholds, SHA digest                   | `TARGET_URL=... CANDIDATE_SHA=... pnpm test:load:staging` | D    | BLOCKED_EXTERNAL_RECEIPT |
