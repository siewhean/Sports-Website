...
| QA-010 | Load-test public projection API responses                        | `apps/api/scripts/run-load-public.ts`                                                                                                                         | Component/socket-component runs are developer diagnostics only; exact deployed API/PostgreSQL/Redis receipt remains required | Staging public projection API p95 `<2.5s`, error rate `<=0.1%`, current SLO version, time window, sample/session thresholds, SHA digest | `TARGET_URL=... CANDIDATE_SHA=... pnpm test:load:staging` | D    | BLOCKED_EXTERNAL_RECEIPT |
| QA-011 | Load-test scoring writes                                         | `apps/api/scripts/run-load-scoring.ts`                                                                                                                        | Component/socket-component runs are developer diagnostics only; exact deployed API/PostgreSQL/Redis receipts remain required | Scoring p95 `<=500ms` plus separately retained public result-propagation p95 `<2s`; both exact-SHA, bounded-window and digest-bound     | `TARGET_URL=... CANDIDATE_SHA=... pnpm test:load:staging` | D    | BLOCKED_EXTERNAL_RECEIPT |
