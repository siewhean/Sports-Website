...
| QA-010 | Load-test public pages                                           | `scripts/run-load-public.ts` (summary in `artifacts/qa-010-load-public-summary.json`)                                                                         | In-process Fastify workload verified (p95: 1.28ms vs <2500ms SLA); live deployment validation pending | Built real HTTP spectator load benchmark: competition, schedule, standings, brackets, search across 1x, 2x, 5x tiers             | `pnpm test:load:public`                           | D    | READY_FOR_LOCAL_PILOT    |
| QA-011 | Load-test scoring writes                                         | `scripts/run-load-scoring.ts` (summary in `artifacts/qa-011-load-scoring-summary.json`)                                                                       | In-process Fastify workload verified (p95: 1.66ms vs <500ms SLA); live deployment validation pending  | Built real HTTP scoring-write load benchmark: concurrent writes, finalisation, standings recompute across 1x, 2x, 5x tiers       | `pnpm test:load:scoring`                          | D    | READY_FOR_LOCAL_PILOT    |
...
| QA-024 | Validate SLO targets against pilot measurements                  | `artifacts/qa-010-load-public-summary.json`, `artifacts/qa-011-load-scoring-summary.json`                                                                     | Pending live telemetry during national pilot                                                          | Pre-pilot Fastify load baseline recorded (scoring p95: 1.66ms, public p95: 1.28ms). Validate against live telemetry during pilot | Load test suite + pilot telemetry                 | E    | READY_FOR_NATIONAL_PILOT |
