...
async function main() {
  ...
  try {
    ...
    const persistedCounts = await sql`
      SELECT
    ...
    assert(firstFailure.last_error?.includes("ECONNREFUSED"), "first failure was not a refused SMTP connection");
...
function captureWorkerLog(chunk) {
...
async function waitForMailpit(timeoutMs) {
...
async function readOutbox(sqlClient, idempotencyKey) {
...
async function waitForOutbox(sqlClient, idempotencyKey, predicate, timeoutMs, label) {
  const deadline = Date.now() + timeoutMs;
  let last = null;
  while (Date.now() < deadline) {
