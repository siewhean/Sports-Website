...
test "$env_mode" = "600"
if grep -Ev '^[[:space:]]*(#|$)' infra/oci/.env.oci | grep -Fq "CHANGE_ME"; then
...
for expected in \
  ...
  if ! grep -Fxq "      $expected" <<<"$compose_config"; then
...
for attempt in $(seq 1 60); do
  ...
  if [ "$attempt" -eq 60 ]; then
    echo "OCI API did not become ready after 60 bounded attempts" >&2
...
web_build_id="$(curl --connect-timeout 5 --max-time 15 --fail --silent --show-error -D - -o /dev/null "https://${OCI_PUBLIC_HOSTNAME}/" | awk 'tolower($1) == "x-matchday-build-id:" { print $2 }' | tr -d '\r\n')"
test "$web_build_id" = "$CANDIDATE_SHA"
worker_id="$(docker compose --env-file infra/oci/.env.oci -f infra/oci/compose.yaml ps -q worker)"
test -n "$worker_id"
for observation in 1 2 3; do
  ...
  if test "$worker_state" != "running 0"; then
    echo "OCI worker failed startup stability check: $worker_state" >&2
    exit 1
  fi
  sleep 5
done
