...
describe("Gate C C4 repair persistence migration", () => {
  it("upgrades populated C4 repair history through 0045 and 0046 without rewriting C3 audit or outbox bytes", async () => {
  ...
  it("adds immutable division-scoped public truth and export metadata without mutating retained history", async () => {
    ...
    expect(result.current).toContain("0037_gate_c_repair_public_truth_exports.sql");
    expect(result.current).toContain("0041_gate_c_repair_append_only_compatibility.sql");
    expect(result.current).toContain("0045_gate_c_atomic_result_repair_cases.sql");
    expect(result.current).toContain("0046_gate_c_assign_result_repair_parent.sql");
