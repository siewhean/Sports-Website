...
const migrationName = "0036_gate_c_offline_replay.sql";
const gateSubsequentMigrationNames = [
...
describe("Gate C C3 migration", () => {
  it("preserves historical audit and outbox bytes while adding offline authority", async () => {
    ...
    await Promise.all(gateSubsequentMigrationNames.map((name) => rm(path.join(copiedDirectory, name))));
