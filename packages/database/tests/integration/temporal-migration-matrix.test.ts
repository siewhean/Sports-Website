...
describeInfrastructure("Temporal Database Migration Matrix (R3)", () => {
  ...
  // Scenario 1: Empty DB
  // ---------------------------------------------------------------------------
  it("Scenario 1: Empty DB - applies all 55 migrations idempotently from clean state", async () => {
    const schema = await createIsolatedSchema("s1_empty");
    const allExpectedMigrations = (await readdir(migrationsDirectory))
      .filter((name) => /^\d{4}_[a-z0-9_]+\.sql$/u.test(name))
      .sort();
    expect(allExpectedMigrations).toHaveLength(55);

    // 1st run: all 55 migrations apply
    ...
    try {
      ...
      // Verify schema_migrations table has all 55 migrations with valid SHA256 checksums
      const recordedMigrations = await sql<{ name: string; checksum: string }[]>`
        SELECT name, checksum FROM schema_migrations ORDER BY name
      `;
      expect(recordedMigrations).toHaveLength(55);
  ...
  it("Scenario 3: Current-main DB - upgrades populated 0035 state forward through 0036..0052", async () => {
    ...
    try {
      ...
      // 4. Now copy and apply forward Gate C and Phase 6 migrations 0036..0055
      const forwardGateCMigrations = allMigrations.filter((name) => name >= "0036_gate_c_offline_replay.sql");
      expect(forwardGateCMigrations).toHaveLength(20);
