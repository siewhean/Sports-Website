...
describe("Gate C C4 append-only compatibility bridge", () => {
  it("sorts before the historical trigger reference without rewriting the applied migration", async () => {
    const bridge = "0041_gate_c_repair_append_only_compatibility.sql";
    const adjustment = "0042_gate_c_repair_schedule_adjustments.sql";
    ...
    expect(bridge.localeCompare("0045_gate_c_atomic_result_repair_cases.sql")).toBeLessThan(0);
