-- format_revisions.definition stores the graph itself, not a builder document.
-- Keep the unseeded-entry authority mapping aligned with that persisted shape.
CREATE OR REPLACE FUNCTION phase4_match_possible_entries(target_match uuid) RETURNS TABLE(entry_id uuid) AS $$
  ...
  format_entry_counts AS (
    SELECT revision.id AS format_revision_id,
      (revision.definition->>'entryCount')::integer AS entry_count
