...
function isSearchFailureError(error: unknown): error is Error {
  return (
...
function matchesByDivision(
  problem: ScheduleProblem,
): readonly Readonly<{ divisionId: string; matches: SchedulingMatch[] }>[] {
  ...
  return [...grouped.entries()]
    ...
    .sort(
      (left, right) => right.matches.length - left.matches.length || left.divisionId.localeCompare(right.divisionId),
    );
}
...
export function generateConstraintAwareSchedule(problem: ScheduleProblem): ScheduleAssignment[] {
  ...
  const attempts =
    problem.objective === "fastest" ? [problem] : [problem, { ...problem, objective: "fastest" as const }];
