  ...
  for (const line of lines) {
    ...
    for (const [index, text] of wrapped.entries()) {
      prepared.push({
        ...
        height: line.lineHeight ?? Math.max(defaultLineHeight, size + 4),
      });
    }
  }
  return prepared;
  ...
  for (const division of document.divisions) {
    ...
    for (const match of division.matches) {
      ...
      lines.push({
        ...
        size: 9,
        // Schedule rows need enough separation to remain legible when printed
        // in greyscale and to prevent dense, error-prone emergency documents.
        lineHeight: 19,
      });
    }
  }
